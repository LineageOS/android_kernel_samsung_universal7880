	...
	unsigned long data0, unsigned int data1)
{
	...
	if (ret < 0) {
		dev_err(dsim->dev, "ID: %2X: MIPI DSIM command write Timeout! 0x%2x\n",
				data_id, (data_id == MIPI_DSI_GENERIC_LONG_WRITE || data_id == MIPI_DSI_DCS_LONG_WRITE) ? *(u8 *)data0 : (unsigned int)data0);
...
static irqreturn_t dsim_interrupt_handler(int irq, void *dev_id)
{
	...
	if (int_src & DSIM_INTSRC_FRAME_DONE)
		...
		dev_dbg(dsim->dev, "DSIM FRAME_DONE was detected!\n");
...
static int dsim_probe(struct platform_device *pdev)
{
	...
	* point, it is too early to handle pending interrupt in kernel.
	*/
