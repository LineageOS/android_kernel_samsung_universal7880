/*
 * drivers/video/decon_7880/panels/s6e3ha2_lcd_ctrl.c
 *
...
struct lcd_info {
	...
	unsigned char			(*vint_table)[AID_CMD_CNT];
...
static void dsim_panel_vint_ctrl(struct lcd_info *lcd, u8 force)
{
	u8 *vint = NULL;
	int ret = 0;
	union vint_info vint_value;

	if (lcd->temperature > -20)
		vint = lcd->vint_table[lcd->bl];
	else
		vint = lcd->vint_table[IBRIGHTNESS_600NIT];

	if (vint == NULL) {
		dev_err(&lcd->ld->dev, "%s: failed to get vint value\n", __func__);
		goto exit;
	}

	vint_value.vint_1 = vint[LDI_OFFSET_VINT_1];
	vint_value.vint_2 = vint[LDI_OFFSET_VINT_2];

	if (force)
		goto vint_update;
	else if (lcd->current_vint.value != vint_value.value)
		goto vint_update;
	else
		goto exit;

vint_update:
	DSI_WRITE(vint, VINT_CMD_CNT);
	lcd->current_vint.value = vint_value.value;
	dev_info(&lcd->ld->dev, "vint: %x\n", lcd->current_vint.value);
...
static int dsim_panel_set_acl(struct lcd_info *lcd, int force)
{
	...
	if (LEVEL_IS_ACL_OFF(index_brightness_table[lcd->bl]) && !lcd->adaptive_control)
		level = ACL_STATUS_0P;
...
static int low_level_set_brightness(struct lcd_info *lcd, int force)
{
	...
	DSI_WRITE(SEQ_GAMMA_UPDATE_L, ARRAY_SIZE(SEQ_GAMMA_UPDATE_L));
...
static int s6e3ha2_read_info(struct lcd_info *lcd, u8 reg, u32 len, u8 *buf)
{
	int ret = 0, i;
...
static int s6e3ha2_read_id(struct lcd_info *lcd)
{
	int ret = 0;
	struct panel_private *priv = &lcd->dsim->priv;

	ret = s6e3ha2_read_info(lcd, LDI_REG_ID, LDI_LEN_ID, lcd->id);
	if (ret < 0) {
		priv->lcdConnected = 0;
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
...
static int s6e3ha2_read_mtp(struct lcd_info *lcd, unsigned char *mtp)
{
	int ret = 0;
	unsigned char buf[LDI_GPARA_DATE + LDI_LEN_DATE] = {0, };

	ret = s6e3ha2_read_info(lcd, LDI_REG_MTP, ARRAY_SIZE(buf), buf);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}
...
static int s6e3ha2_read_coordinate(struct lcd_info *lcd)
{
	...
	ret = s6e3ha2_read_info(lcd, LDI_REG_COORDINATE, LDI_LEN_COORDINATE, buf);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}
...
static int s6e3ha2_read_chip_id(struct lcd_info *lcd)
{
	int ret = 0;

	ret = s6e3ha2_read_info(lcd, LDI_REG_CHIP_ID, LDI_LEN_CHIP_ID, lcd->code);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}
...
static int s6e3ha2_read_elvss(struct lcd_info *lcd, unsigned char *buf)
{
	int ret = 0;

	ret = s6e3ha2_read_info(lcd, LDI_REG_ELVSS, LDI_LEN_ELVSS, buf);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}
...
static int s6e3ha2_init_elvss(struct lcd_info *lcd, u8 *elvss_data)
{
	int i, temp, ret = 0;
...
static int s6e3ha2_read_hbm(struct lcd_info *lcd, unsigned char *buf)
{
	int ret = 0;

	DSI_WRITE(SEQ_HBM_GLOBAL, ARRAY_SIZE(SEQ_HBM_GLOBAL));

	ret = s6e3ha2_read_info(lcd, LDI_REG_HBM, LDI_LEN_HBM, buf);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}
...
#if 0
static int s6e3ha2_read_status(struct lcd_info *lcd)
{
	int ret = 0;
	...
	ret = s6e3ha2_read_info(lcd, LDI_REG_RDDPM, LDI_LEN_RDDPM, rddpm);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}

	ret = s6e3ha2_read_info(lcd, LDI_REG_RDDSM, LDI_LEN_RDDSM, rddsm);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}
	ret = s6e3ha2_read_info(lcd, LDI_REG_ESDERR, LDI_LEN_ESDERR, esderr);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}
...
static int s6e3ha2_init_hbm_elvss(struct lcd_info *lcd, u8 *elvss_data)
{
	int i, temp, ret = 0;
...
//	u8 v255[CI_MAX][2] = {{0,}, };
	...
	memcpy(&tmp[0], hbm_data, LDI_LEN_HBM);
	...
	/* 6th: B4h 4th para */
#if 0
	v255[CI_RED][0] = get_bit(hbm_data[0], 0, 1);
	v255[CI_RED][1] = hbm_data[1];

	v255[CI_GREEN][0] = get_bit(hbm_data[2], 0, 1);
	v255[CI_GREEN][1] = hbm_data[3];

	v255[CI_BLUE][0] = get_bit(hbm_data[4], 0, 1);
	v255[CI_BLUE][1] = hbm_data[5];
...
static int s6e3ha2_init_hbm_interpolation(struct lcd_info *lcd)
{
	unsigned char hbm_data[LDI_LEN_HBM] = {0,};
	int ret = 0;

	ret |= s6e3ha2_read_hbm(lcd, hbm_data);
...
static int s6e3ha2_exit(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3ha2_displayon(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3ha2_init(struct lcd_info *lcd)
{
	int ret = 0;
	...
	/* 9. Interface setting */
	DSI_WRITE(SEQ_DUAL_DSI_1, ARRAY_SIZE(SEQ_DUAL_DSI_1));
	...
	DSI_WRITE(SEQ_TSP_TE, ARRAY_SIZE(SEQ_TSP_TE));
	DSI_WRITE(SEQ_PENTILE_SETTING, ARRAY_SIZE(SEQ_PENTILE_SETTING));
	DSI_WRITE(SEQ_POC_SETTING1, ARRAY_SIZE(SEQ_POC_SETTING1));
	DSI_WRITE(SEQ_POC_SETTING2, ARRAY_SIZE(SEQ_POC_SETTING2));
	DSI_WRITE(SEQ_OSC_SETTING1, ARRAY_SIZE(SEQ_OSC_SETTING1));
	DSI_WRITE(SEQ_OSC_SETTING2, ARRAY_SIZE(SEQ_OSC_SETTING2));
	DSI_WRITE(SEQ_OSC_SETTING3, ARRAY_SIZE(SEQ_OSC_SETTING3));
...
static int s6e3ha2_read_init_info(struct lcd_info *lcd, unsigned char *mtp)
{
	int ret = 0;
	...
	ret |= s6e3ha2_read_elvss(lcd, elvss_data);

	ret |= s6e3ha2_init_elvss(lcd, elvss_data);
	ret |= s6e3ha2_init_hbm_elvss(lcd, elvss_data);
	ret |= s6e3ha2_init_hbm_interpolation(lcd);
...
static int s6e3ha2_probe(struct dsim_device *dsim)
{
	int ret = 0;
	...
	ret = s6e3ha2_read_init_info(lcd, mtp);
	if (!priv->lcdConnected) {
...
{
	struct panel_private *priv = &(lcd->priv);
	int ret = 0;
