...
static int mobicore_start(void)
{
	...
	if (ret) {
		...
		/* ExySp: for debug for mobicore start fail */
		panic("panic because of mobicore start fail");
		goto err_log;
	}
	 ...
	 */
	if (mc_switch_core(NONBOOT_LITTLE_CORE))
		mc_dev_info("mc_switch_core failure");
